...
import { FormatProduct, NewProduct } from "../domain/product";
...
export default class ProductStore extends Base {
  @observable products = [];
  @observable total = 0;
  @observable type = "";
  @observable url = "";
  ...
  @action setData({ products = [], total, type, url, current }) {
  ...
  @action updateUpvotes = ({ count, isLiked, id: productID }) => {
    this.products = [
      ...this.products.map((p) => {
        if (p.id === productID) {
          p.upvotes = count;
